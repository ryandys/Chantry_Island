<?php
	...
	function editUser($id, $name, $email, $password) {
		...
		$updatestring = "UPDATE tbl_user SET user_name='{$name}', user_email='{$email}', user_pass='{$EncPassword}' WHERE user_id={$id}";
	...
	function createUser($name, $email, $level) {
		...
		$salt = 'hilfue2fnlfmkon4iu4f6r7fherwuvnijvhcibchieqiubiebuvijevijqevjjewaa';
		$EncPassword = hash('sha256', $salt.$password);
		$userstring = "INSERT INTO tbl_user VALUES(NULL,'{$name}','{$EncPassword}','{$ip}','{$level}','{$email}','0','0')";
		//echo $userstring;
		$userquery = mysqli_query($link, $userstring);
		if($userquery) {
